...
import { locationInsideOwnBuilding } from '../model/buildingHorizon';
...
import { useSurfaceLocal } from './useSurfaceModel';

// ─────────────────────────────────────────────
// LASER-SCAN (swissSURFACE3D) HORIZON: LOADER
...
// While the location lies inside its building (an address point before the site plan's «Übernehmen»,
// buildingHorizon.ts locationInsideOwnBuilding) nothing is computed or downloaded (status 'waiting'), and a
// download waits for a building import under way (its buildings decide that).
// The readers and the loader's small state store are hooks/useSurfaceModel.ts; <DataLoader/> loads this
// module (a chunk of its own) the first time the laser scan is on in a session.
// ─────────────────────────────────────────────
...
export interface SurfacePlan {
  ...
  sweep: ObserverSet;
  /**
   * The location lies inside its building (a picked address before «Übernehmen» in the site plan,
   * buildingHorizon.ts locationInsideOwnBuilding): nothing is computed or downloaded, status 'waiting'.
   */
  waiting: boolean;
}
...
/**
 * Resolves once no building import is requested (uiStore.surroundingsImport, until <DataLoader/> has loaded
 * the import and taken it) or running (buildingImportStore status 'loading'); rejects when `signal` aborts
 * first.
 */
function buildingImportSettled(signal: AbortSignal): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    const settled = (): boolean =>
      useUiStore.getState().surroundingsImport === null &&
      useBuildingImportStore.getState().status !== 'loading';
    if (settled()) {
      resolve();
      return;
    }
    ...
    // Checked again a microtask later: taking the request (uiStore) and starting the import ('loading') are
    // two store updates in one call.
    const check = (): void => {
    ...
    const unsubscribe = useBuildingImportStore.subscribe(check);
...
export function useSurfaceModelLoader(): void {
  ...
  useEffect(() => {
    ...
    if (!enabled || jobKey === null || waiting) {
      ...
      // Inside the own building (an address point before the site plan's «Übernehmen»): the scan would see
      // that building all round; it loads for the balcony once the location is on the facade.
      setSurface(enabled && waiting ? { ...INITIAL_SURFACE, status: 'waiting' } : INITIAL_SURFACE);
      return;
    }
  ...
  }, [enabled, jobKey, currentKey, attempt, loaded, waiting]);
  ...
  const download = enabled && !waiting && dataKey !== null && wanted === dataKey;
  useEffect(() => {
    ...
    const run = async (): Promise<void> => {
      ...
      try {
        ...
        // A building import under way (after an address pick) decides whether the location lies inside
        // its building: then nothing is downloaded for it (status 'waiting').
        await buildingImportSettled(signal);
      } catch {
        return; // aborted
      }
      // The site and observers at the start (later observers are computed from memory when it is done).
      const plan = surfacePlan(useConfigStore.getState().config);
      if (plan.dataKey !== dataKey || plan.waiting) return;
      const cached = readSurfaceCache(plan.jobKey);
      const todo = missing(plan.current, { ...(cached?.horizons ?? {}), ...publishedHorizons(plan) });
      let result: DsmJobResult;
      try {
...
/** The loader as a component (<DataLoader/> mounts it lazily). Renders nothing. */
export function SurfaceModelLoader(): null {
