...
const PANEL_LENGTH = 113.4; // cm, the "height" when tilted
const PANEL_WIDTH = 176.2; // cm, single panel
const PANEL_THICKNESS = 3; // cm
const NUM_PANELS = 2;
const TOTAL_PANEL_WIDTH = PANEL_WIDTH * NUM_PANELS; // 352.4 cm
const BALCONY_HEIGHT = 280;
const RAILING_HEIGHT = 100;
const LATITUDE = 47.1;
const FACADE_AZIMUTH = 213;

const toRad = (deg) => (deg * Math.PI) / 180;
const toDeg = (rad) => (rad * 180) / Math.PI;

function getPanelGeometry(tilt) {
  const pH = PANEL_LENGTH * Math.sin(toRad(tilt));
  const pD = PANEL_LENGTH * Math.cos(toRad(tilt));
  const vGap = BALCONY_HEIGHT - pH;
  const crit = toDeg(Math.atan(vGap / pD));
  return { panelH: pH, panelD: pD, verticalGap: vGap, criticalAngle: crit };
}

function getSolarPosition(dayOfYear, hour) {
...
function getProfileAngle(alt, az) {
  const diff = toRad(az - FACADE_AZIMUTH);
...
function getRelativeYield(tilt) {
  let total = 0;
  for (let m = 1; m <= 12; m++) {
    const doy = getDayOfYear(m, 15);
    for (let h = 5; h <= 21; h += 0.25) {
      const sol = getSolarPosition(doy, h);
      if (sol.altitude <= 2) continue;
      const cosInc = Math.sin(toRad(sol.altitude)) * Math.cos(toRad(tilt)) + Math.cos(toRad(sol.altitude)) * Math.sin(toRad(tilt)) * Math.cos(toRad(sol.azimuth - FACADE_AZIMUTH));
...
/* ========== TOP-DOWN VIEW ========== */
function TopDownView({ tilt, hour, season, allSeasons }) {
  const geo = getPanelGeometry(tilt);
  const vw = 520;
  const vh = 440;
  const cx = vw / 2;
  const cy = vh / 2 + 20;
  ...
  const currentSol = getSolarPosition(season.day, hour);
  const profileAngle = currentSol.altitude > 0 ? getProfileAngle(currentSol.altitude, currentSol.azimuth) : null;
  ...
  const facadeNorm = azToXY(FACADE_AZIMUTH, radius * 0.55);
  const facadePerp = FACADE_AZIMUTH + 90;

  const allPaths = allSeasons.map(sd => {
    const pts = [];
    for (let h = 4; h <= 22; h += 0.15) {
      const s = getSolarPosition(sd.day, h);
  ...
  const arcStart = FACADE_AZIMUTH - 90;
  const arcEnd = FACADE_AZIMUTH + 90;
  ...
  const hourMarkers = [];
  for (let h = 5; h <= 21; h++) {
    const s = getSolarPosition(season.day, h);
    if (s.altitude <= 0) continue;
    const r = radius * (1 - s.altitude / 90 * 0.4);
  ...
  // Panel outline in top-down: a rectangle outward from railing
  // Scale: use a consistent px/cm for the building area
  const topSc = 0.14; // px per cm for top-down detail
  const panelDepth = geo.panelD * topSc; // how far out
  const panelW = TOTAL_PANEL_WIDTH * topSc; // total width
  const normR = toRad(FACADE_AZIMUTH);
  const perpR = toRad(facadePerp);

  // Railing center (just outside building)
  const railDist = 30; // px from center
  const railCx = cx + railDist * Math.sin(normR);
  const railCy = cy - railDist * Math.cos(normR);

  // Panel 4 corners: from railing, extends outward (in facade normal direction)
  function panelCorners() {
    const hw = panelW / 2;
    // Along facade (perpendicular): ±hw
    // Outward (normal direction): 0 to panelDepth
    const corners = [
      [0, -hw], [0, hw], [panelDepth, hw], [panelDepth, -hw]
    ];
  ...
  // Panel divider line (between the two panels)
  const divStart = {
    x: railCx + 0 * Math.sin(normR),
    y: railCy - 0 * Math.cos(normR),
  };
  const divEnd = {
    x: railCx + panelDepth * Math.sin(normR),
    y: railCy - panelDepth * Math.cos(normR),
  };
  ...
  return (
    <svg viewBox={`0 0 ${vw} ${vh}`} style={{ width: "100%", background: "#0f172a", borderRadius: 12 }}>
      ...
      {/* Building */}
      {(() => {
        const angle = toRad(FACADE_AZIMUTH);
        const hw = bldgW / 2;
        const hh = 35;
      ...
      <text x={facadeNorm.x + 8} y={facadeNorm.y - 6} fill="#f97316" fontSize={9} fontWeight={600}>{FACADE_AZIMUTH}° SSW</text>

      {/* Panels — top-down rectangle */}
      <polygon points={panelPts} fill="#22c55e" opacity={0.25} stroke="#4ade80" strokeWidth={1.5} />
      {/* Divider between 2 panels */}
      <line x1={divStart.x} y1={divStart.y} x2={divEnd.x} y2={divEnd.y} stroke="#4ade80" strokeWidth={0.8} opacity={0.5} />
      {/* Width label */}
      <text x={widthLabelPos.x} y={widthLabelPos.y} fill="#4ade80" fontSize={8} textAnchor="middle" dominantBaseline="middle"
        transform={`rotate(${FACADE_AZIMUTH + 90}, ${widthLabelPos.x}, ${widthLabelPos.y})`}>
        2×176 = 352 cm
      </text>
      {/* Depth label */}
      <text x={depthMid.x} y={depthMid.y} fill="#4ade80" fontSize={8} textAnchor="middle" dominantBaseline="middle"
        transform={`rotate(${FACADE_AZIMUTH}, ${depthMid.x}, ${depthMid.y})`}>
      ...
      <text x={34} y={vh - 44} fill="#f97316" fontSize={9}>Fassaden-Normale (213°)</text>
      <rect x={12} y={vh - 38} width={18} height={6} fill="#22c55e" opacity={0.25} stroke="#4ade80" strokeWidth={1} />
      <text x={34} y={vh - 30} fill="#22c55e" fontSize={9}>Panels (2× 176×113 cm)</text>
...
/* ========== FRONTAL VIEW ========== */
function FrontalView({ tilt, hour, season, allSeasons }) {
  const geo = getPanelGeometry(tilt);
  const vw = 600;
  const vh = 440;
  const groundY = 400;
  const roofY = 225;
  const facadeTop = roofY;

  // Scale building so panels fit proportionally
  // Panels are 352.4 cm wide. Building facade should be a bit wider.
  const facadeSc = 0.055; // cm to px
  const panelVisTotalW = TOTAL_PANEL_WIDTH * facadeSc; // ~19.4px... too small
  // Use a better scale for the building section
  const bsc = 0.38; // building scale: px per cm for frontal proportions
  const panelTotalW = TOTAL_PANEL_WIDTH * bsc; // 352.4 * 0.38 = ~134px
  const singlePanelW = PANEL_WIDTH * bsc;
  const facadeW = panelTotalW + 40; // building slightly wider than panels
  ...
  const vertSc = 0.19;
  const upperBalcY = facadeTop + 36;
  const lowerBalcY = upperBalcY + BALCONY_HEIGHT * vertSc;
  const railH = RAILING_HEIGHT * vertSc;
  const panelVisH = geo.panelH * vertSc;

  const skyRadius = 240;
  const skyCenterX = vw / 2;
  const skyCenterY = roofY;

  function sunToXY(alt, az) {
    const relAz = az - FACADE_AZIMUTH;
  ...
  // Helper to draw a balcony with 2 panels
  function renderBalcony(balcY, panelColor, panelStroke, label) {
    return (
      <>
        ...
        {/* Panel 1 */}
        <rect x={panelLeft} y={balcY - railH} width={singlePanelW - 2} height={panelVisH}
          fill={panelColor} opacity={0.7} stroke={panelStroke} strokeWidth={1} rx={2} />
        {/* Panel 2 */}
        <rect x={panelLeft + singlePanelW + 2} y={balcY - railH} width={singlePanelW - 2} height={panelVisH}
          fill={panelColor} opacity={0.7} stroke={panelStroke} strokeWidth={1} rx={2} />
        {/* Label */}
  ...
  return (
    <svg viewBox={`0 0 ${vw} ${vh}`} style={{ width: "100%", background: "#0f172a", borderRadius: 12 }}>
      ...
      <text x={vw - 55} y={roofY - 6} fill="#475569" fontSize={10} fontWeight={600}>→ O</text>
      <text x={30} y={roofY - 6} fill="#475569" fontSize={10} fontWeight={600}>W ←</text>
      <text x={skyCenterX - 4} y={roofY - 6} fill="#64748b" fontSize={9}>S</text>
      ...
      {/* Windows */}
      {[0, 1].map(row => {
        const wy = facadeTop + 10 + row * (BALCONY_HEIGHT * vertSc);
      ...
      {/* Balconies with panels */}
      {renderBalcony(upperBalcY, "#f97316", "#fb923c", "2× Oberes Panel (176×113 cm)")}
      {renderBalcony(lowerBalcY, "#22c55e", "#4ade80", "2× Unteres Panel (176×113 cm)")}

      {/* Panel width annotation */}
      <line x1={panelLeft} y1={lowerBalcY + 10} x2={panelLeft + panelTotalW} y2={lowerBalcY + 10}
        stroke="#94a3b8" strokeWidth={0.8} />
      <line x1={panelLeft} y1={lowerBalcY + 6} x2={panelLeft} y2={lowerBalcY + 14} stroke="#94a3b8" strokeWidth={0.8} />
      <line x1={panelLeft + panelTotalW} y1={lowerBalcY + 6} x2={panelLeft + panelTotalW} y2={lowerBalcY + 14} stroke="#94a3b8" strokeWidth={0.8} />
      <text x={vw / 2} y={lowerBalcY + 22} fill="#94a3b8" fontSize={8} textAnchor="middle">352 cm (2×176)</text>
        ...
        <>
          ...
          {!isBehind && profileAngle !== null && profileAngle > 0 && (
            <line x1={currentPos.x} y1={currentPos.y + 14}
              x2={vw / 2} y2={lowerBalcY - railH + panelVisH / 2}
...
/* ========== PROFILE (SIDE) VIEW ========== */
function ProfileDiagram({ tilt, profileAngle, season }) {
  const { panelH, panelD, criticalAngle } = getPanelGeometry(tilt);
  const sc = 0.28;
  const wallX = 130;
  const upperSlabY = 70;
  const lowerSlabY = upperSlabY + BALCONY_HEIGHT * sc;
  const railOffsetX = 55;
  const railX = wallX + railOffsetX;
  const upperRailTopY = upperSlabY - RAILING_HEIGHT * sc;
  const lowerRailTopY = lowerSlabY - RAILING_HEIGHT * sc;
  ...
  // Panel thickness in side view
  const thickPx = PANEL_THICKNESS * sc;
  ...
  return (
    <svg viewBox="0 0 480 340" style={{ width: "100%", background: "#0f172a", borderRadius: 12 }}>
      ...
      <text x={wallX - 78} y={(upperSlabY + lowerSlabY) / 2 + 4} fill="#94a3b8" fontSize={9} fontWeight={600}>280 cm</text>

      {/* Upper panel — with thickness */}
      {(() => {
      ...
      {(() => {
        ...
        const vGap = BALCONY_HEIGHT - panelH;
        return (
          <>
      ...
      <text x={12} y={lowerSlabY + panelH * sc + 44} fill="#64748b" fontSize={9}>
        Panel: {PANEL_LENGTH}×{PANEL_WIDTH}×{PANEL_THICKNESS} cm · Querschnitt (Breite nicht sichtbar)
      </text>
    </svg>
  );
}
...
/* ========== TILT COMPARISON ========== */
function TiltComparisonChart({ currentTilt }) {
  const tilts = [25, 30, 35, 40, 45, 50, 55, 60, 65, 70];
  const data = useMemo(() => {
    const baseY = getRelativeYield(40);
    return tilts.map(t => {
      const geo = getPanelGeometry(t);
      const relY = getRelativeYield(t) / baseY;
      const doy = getDayOfYear(6, 21);
      let sH = 0, maxP = 0;
      for (let h = 5; h <= 21; h += 0.25) {
        const sol = getSolarPosition(doy, h);
        if (sol.altitude <= 0) continue;
        const p = getProfileAngle(sol.altitude, sol.azimuth);
...
/* ========== MONTHLY YIELD ========== */
function MonthlyYieldChart({ currentTilt }) {
    ...
    for (let m = 1; m <= 12; m++) {
      ...
      for (const t of compareTilts) {
        ...
        for (let h = 5; h <= 21; h += 0.25) {
          const sol = getSolarPosition(doy, h);
          if (sol.altitude <= 2) continue;
          const cosInc = Math.sin(toRad(sol.altitude)) * Math.cos(toRad(t)) + Math.cos(toRad(sol.altitude)) * Math.sin(toRad(t)) * Math.cos(toRad(sol.azimuth - FACADE_AZIMUTH));
...
export default function ShadowAnalysis() {
  ...
  const solar = getSolarPosition(season.day, hour);
  const profileAngle = solar.altitude > 0 ? getProfileAngle(solar.altitude, solar.azimuth) : null;
  const isShadow = profileAngle !== null && profileAngle >= geo.criticalAngle;
  ...
  const yearAnalysis = useMemo(() => {
    ...
    for (let m = 1; m <= 12; m++) {
      ...
      for (let h = 6; h <= 20; h += 0.25) {
        const sv = getSolarPosition(doy, h);
        if (sv.altitude <= 0) continue;
        const p = getProfileAngle(sv.altitude, sv.azimuth);
        if (p !== null && p > maxP) maxP = p;
        if (p !== null && p >= geo.criticalAngle) sH += 0.25;
      }
  ...
  return (
    <div style={{ fontFamily: "'Inter', system-ui, sans-serif", background: "#0f172a", color: "#e2e8f0", minHeight: "100vh", padding: "24px 16px" }}>
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, marginBottom: 4 }}>Verschattungsanalyse — Interaktiv</h1>
        <p style={{ color: "#94a3b8", fontSize: 13, marginBottom: 20 }}>
          Münchenbuchsee · 47.1°N · Fassade 213° SSW · 2×{PANEL_WIDTH}×{PANEL_LENGTH} cm Panels · Balkonhöhe 280 cm
        </p>

        {/* Tilt slider */}
        <div style={{ background: "linear-gradient(135deg, #1e1b4b, #312e81)", borderRadius: 12, padding: "16px 20px", marginBottom: 16, border: "1px solid #4f46e5" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: "#a5b4fc" }}>Panelneigung</span>
            <span style={{ fontSize: 28, fontWeight: 800, color: "#f8fafc", fontVariantNumeric: "tabular-nums" }}>{tilt}°</span>
          </div>
          <input type="range" min={20} max={75} step={1} value={tilt} onChange={(e) => setTilt(parseInt(e.target.value))} style={{ width: "100%", accentColor: "#818cf8" }} />
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "#6366f1", marginTop: 4 }}><span>20° (flach)</span><span>75° (steil)</span></div>
        </div>

        {/* Result box */}
        <div style={{
          background: yearAnalysis.every(r => r.maxProfile < geo.criticalAngle) ? "linear-gradient(135deg, #064e3b, #065f46)" : "linear-gradient(135deg, #78350f, #92400e)",
          borderRadius: 12, padding: "16px 20px", marginBottom: 16,
          border: `1px solid ${yearAnalysis.every(r => r.maxProfile < geo.criticalAngle) ? "#10b981" : "#f59e0b"}`
        }}>
          <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
            <div>
              <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: 1, color: yearAnalysis.every(r => r.maxProfile < geo.criticalAngle) ? "#6ee7b7" : "#fde68a" }}>Kritischer Profilwinkel</div>
              <div style={{ fontSize: 26, fontWeight: 800, color: "#fbbf24" }}>{geo.criticalAngle.toFixed(1)}°</div>
            </div>
            <div>
              <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: 1, color: yearAnalysis.every(r => r.maxProfile < geo.criticalAngle) ? "#6ee7b7" : "#fde68a" }}>Panel ragt</div>
              <div style={{ fontSize: 14, fontWeight: 600, color: "#f8fafc" }}>{geo.panelD.toFixed(1)} cm raus · {geo.panelH.toFixed(1)} cm runter</div>
            </div>
            <div>
              <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: 1, color: yearAnalysis.every(r => r.maxProfile < geo.criticalAngle) ? "#6ee7b7" : "#fde68a" }}>Verschattung</div>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <input type="range" min={5} max={21} step={0.25} value={hour} onChange={(e) => setHour(parseFloat(e.target.value))} style={{ width: "100%", accentColor: season.color, marginTop: 4 }} />
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {activeViews.frontal && <FrontalView tilt={tilt} hour={hour} season={season} allSeasons={seasonDays} />}
          {activeViews.topdown && <TopDownView tilt={tilt} hour={hour} season={season} allSeasons={seasonDays} />}
          {activeViews.profile && <ProfileDiagram tilt={tilt} profileAngle={profileAngle !== null && profileAngle > 0 ? profileAngle : null} season={season} />}
        </div>

        <TiltComparisonChart currentTilt={tilt} />
        <MonthlyYieldChart currentTilt={tilt} />
