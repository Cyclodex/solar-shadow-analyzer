...
const de = {
  ...
  desc: (floors: string, theta: string, beta: string, h: string, gap: string, reach: string) =>
    `Schnitt durch ${floors}. Neigung θ ${theta} ab Senkrechte (β ${beta} ab Horizontal), Stockwerkhöhe ${h}, ${gap}, Ausladung ${reach}.`,
  ...
  gap: (v: string) => `Freiraum ${v}`,
  gapDesc: (v: string) => `Freiraum zur Reihe darunter ${v}`,
  overlap: (v: string) => `Überlappung ${v}`,
  reach: (v: string) => `Ausladung ${v}`,
  ...
  shadeNow: (floor: string, pct: string) => `${floor}: ${pct} der Fläche verschattet (exaktes 3D-Modell).`,
...
const messages: Messages<ProfileText> = {
  ...
  en: {
    ...
    desc: (floors, theta, beta, h, gap, reach) =>
      `Section through ${floors}. Tilt θ ${theta} from vertical (β ${beta} from horizontal), floor-to-floor height ${h}, ${gap}, reach ${reach}.`,
    ...
    gap: (v) => `Clearance ${v}`,
    gapDesc: (v) => `clearance to the row below ${v}`,
    overlap: (v) => `Overlap ${v}`,
    reach: (v) => `Reach ${v}`,
    height: (v) => `Floor-to-floor height ${v}`,
    theta: (v) => `θ ${v}`,
    beta: (v) => `β ${v}`,
    legendRay: 'Sun ray (profile angle)',
    legendCritical: 'Critical angle (2D)',
    legendShade: 'Shadow (3D model)',
    shadeNow: (floor, pct) => `${floor}: ${pct} of the area shaded (exact 3D model).`,
